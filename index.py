...
def main():
    ...
    #authenticate user
    login_attempts = 0
    #5. This failed attempts boolean works, but it is reccomended that we remove it as it adds neednless complexity to the code
    # failedAttempt = False
    while login_attempts < 3:
        ...
        # 6. Incorrect auth call
        user = auth.login_again(userName, password)
        if not user:
            login_attempts += 1
            #failedAttempt = True
            message.print("please try again")
